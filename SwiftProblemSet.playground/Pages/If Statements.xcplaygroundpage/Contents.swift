...
func checkForRelease(bugs: Int, music: Bool, levels: Int) {
    if bugs < maxNoBugs && music && levels > minNoLevels {
        release()
    } else {
        if bugs >= maxNoBugs {
            print("\(game) contains to many bugs (\(bugs))")
        }
        if !music {
            print("\(game) doesnt contain any music")
        }
        if levels <= minNoLevels {
            print("\(game) is too easy (only \(levels) levels)")
        }
    }
}
...
func checkTrainingStatus(name: String, bike: Bool, run: Bool) {
    ...
    if bike && run {
        print("\(name) is ready for the race")
    } else if bike {
        print("still some running to do")
    } else if run {
        print("gotta learn how to bike faster")
    } else {
        print("gotta work on your running and on your cycling")
    }
}
